...
public class BlendingHelper
{
	...
	public Color ProfileColorForKey(SkyProfile profile, string key)
	{
		...
		float time = (((Object)(object)profile == (Object)(object)m_State.toProfile) ? 0f : m_State.timeOfDay);
	...
	public void BlendSpherePoint(string key, SpherePoint from, SpherePoint to, float progress)
	{
		...
		Vector3 val = Vector3.Slerp(from.GetWorldDirection(), to.GetWorldDirection(), progress);
		m_State.blendedProfile.GetGroup<SpherePointKeyframeGroup>(key).keyframes[0].spherePoint = new SpherePoint(((Vector3)(ref val)).normalized);
	}
