...
public class RainSplashRenderer : BaseSpriteInstancedRenderer
{
	...
	private void Start()
	{
		...
		{
			Debug.LogError((object)"Can't render rain splashes since GPU instancing is not supported on this platform.");
		...
		{
			Debug.LogError((object)"Can't generate splashes without a RainDepthCamera in the scene");
			((Behaviour)this).enabled = false;
		}
		else
		{
			m_DepthCamera = ((Component)weatherDepthCamera).GetComponent<Camera>();
		}
	}
	...
	protected override bool IsRenderingEnabled()
	{
		...
		if ((Object)(object)base.m_ViewerCamera == (Object)null)
		{
			Debug.LogError((object)"Can't render ground raindrops since no active camera has the MainCamera tag applied.");
			return false;
		}
		return true;
	}
	...
	protected override void PrepareDataArraysForRendering(int instanceId, BaseSpriteItemData data)
	{
		...
		Vector3 val = m_DepthCamera.WorldToScreenPoint(rainSplashData.spritePosition);
		Vector2 depthTextureUV = default(Vector2);
		((Vector2)(ref depthTextureUV))._002Ector(val.x / (float)m_DepthCamera.pixelWidth, val.y / (float)m_DepthCamera.pixelHeight);
	...
	private Vector3 CreateWorldSplashPoint()
	{
		...
		float num = Random.Range(0f, -170f);
		Vector3 val = Quaternion.Euler(new Vector3(0f, num, 0f)) * Vector3.right;
		float num2 = Random.Range(m_SplashAreaStart, m_SplashAreaStart + m_SplashAreaLength);
		Vector3 val2 = ((Vector3)(ref val)).normalized * num2;
		return ((Component)base.m_ViewerCamera).transform.TransformPoint(val2);
	}
}
