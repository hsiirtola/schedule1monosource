...
public class SkyProfile : ScriptableObject
{
	...
	public Material skyboxMaterial
	{
		...
		set
		{
			...
			else if (Object.op_Implicit((Object)(object)m_SkyboxMaterial) && ((Object)m_SkyboxMaterial.shader).name != ((Object)value.shader).name)
			{
				m_SkyboxMaterial = value;
				m_ShaderName = ((Object)value.shader).name;
				ReloadDefinitions();
			}
			else
			{
	...
	{
		NumberKeyframeGroup numberKeyframeGroup = GetGroup<NumberKeyframeGroup>(propertyKey);
		if (numberKeyframeGroup == null)
		{
			Debug.LogError((object)("Can't find number group with property key: " + propertyKey));
			return -1f;
		}
		return numberKeyframeGroup.NumericValueAtTime(timeOfDay);
	}
	...
	public Color GetColorPropertyValue(string propertyKey, float timeOfDay)
	{
		...
		ColorKeyframeGroup colorKeyframeGroup = GetGroup<ColorKeyframeGroup>(propertyKey);
		if (colorKeyframeGroup == null)
		{
			Debug.LogError((object)("Can't find color group with property key: " + propertyKey));
			return Color.white;
		}
		return colorKeyframeGroup.ColorForTime(timeOfDay);
	}
	...
	{
		TextureKeyframeGroup textureKeyframeGroup = GetGroup<TextureKeyframeGroup>(propertyKey);
		if (textureKeyframeGroup == null)
		{
			Debug.LogError((object)("Can't find texture group with property key: " + propertyKey));
			return null;
		}
		return textureKeyframeGroup.TextureForTime(timeOfDay);
	}
	...
	{
		SpherePointKeyframeGroup spherePointKeyframeGroup = GetGroup<SpherePointKeyframeGroup>(propertyKey);
		if (spherePointKeyframeGroup == null)
		{
			Debug.LogError((object)("Can't find a sphere point group with property key: " + propertyKey));
			return null;
		}
		return spherePointKeyframeGroup.SpherePointForTime(timeOfDay);
	}
	...
	{
		BoolKeyframeGroup boolKeyframeGroup = GetGroup<BoolKeyframeGroup>(propertyKey);
		if (boolKeyframeGroup == null)
		{
			Debug.LogError((object)("Can't find boolean group with property key: " + propertyKey));
			return false;
		}
		return boolKeyframeGroup.BoolForTime(timeOfDay);
	}
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			foreach (ProfileGroupDefinition profileGroupDefinition in groups)
			{
				...
				else if (profileGroupDefinition.type == ProfileGroupDefinition.GroupType.Number)
				{
					...
					NumberKeyframeGroup numberKeyframeGroup = keyframeGroups.GetGroup<NumberKeyframeGroup>(profileGroupDefinition.propertyKey);
					numberKeyframeGroup.name = profileGroupDefinition.groupName;
					numberKeyframeGroup.minValue = profileGroupDefinition.minimumValue;
					numberKeyframeGroup.maxValue = profileGroupDefinition.maximumValue;
				}
	...
	{
		TextureKeyframeGroup value = new TextureKeyframeGroup(groupName, new TextureKeyframe((Texture)(object)texture, 0f));
		keyframeGroups[propKey] = value;
	}
	...
	public T GetGroup<T>(string propertyKey) where T : class
	{
		...
		{
			Debug.Log((object)("Key does not exist in sky profile, ignoring: " + propertyKey));
			return null;
		}
	...
	public void SetFeatureEnabled(string featureKey, bool value)
	{
		if (featureKey == null)
		{
			Debug.LogError((object)"Can't set null feature key value");
		}
		else
		{
