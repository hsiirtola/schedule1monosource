...
public class AvatarEmotionManager : MonoBehaviour
{
	...
	private void Start()
	{
		...
		((MonoBehaviour)this).InvokeRepeating("UpdateEmotion", 0f, 0.25f);
	}
	...
	public void UpdateEmotion()
	{
		...
		if (!PlayerSingleton<PlayerCamera>.InstanceExists || !(Vector3.Distance(((Component)this).transform.position, ((Component)PlayerSingleton<PlayerCamera>.Instance).transform.position) > 30f))
		{
	...
	public void ConfigureNeutralFace(Texture2D faceTex, float restingBrowHeight, float restingBrowAngle, Eye.EyeLidConfiguration leftEyelidConfig, Eye.EyeLidConfiguration rightEyelidConfig)
	{
		...
		{
			Debug.LogError((object)"Could not find neutral preset");
			return;
		}
	...
	public virtual void AddEmotionOverride(string emotionName, string overrideLabel, float duration = 0f, int priority = 0)
	{
		...
		if (duration > 0f)
		{
			Coroutine value = ((MonoBehaviour)Singleton<CoroutineService>.Instance).StartCoroutine(RemoveEmotionAfterDuration());
	...
	private void ClearRemovalRoutine(string label)
	{
		...
		if (emotionRemovalRoutines.ContainsKey(label))
		{
			...
			{
				((MonoBehaviour)Singleton<CoroutineService>.Instance).StopCoroutine(emotionRemovalRoutines[label]);
			}
	...
	private void LerpEmotion(AvatarEmotionPreset preset, float animationTime = 0.2f)
	{
		...
		{
			((MonoBehaviour)Singleton<CoroutineService>.Instance).StopCoroutine(emotionLerpRoutine);
		}
		emotionLerpRoutine = ((MonoBehaviour)Singleton<CoroutineService>.Instance).StartCoroutine(Routine());
		IEnumerator Routine()
		{
