...
public class CasinoGamePlayerData
{
	...
	public void SetData<T>(string key, T value, bool network = true)
	{
		if (network)
		{
			if (typeof(T) == typeof(bool))
			{
				Parent.SendPlayerBool(((NetworkBehaviour)Player).NetworkObject, key, (bool)(object)value);
			}
			else if (typeof(T) == typeof(float))
			{
				Parent.SendPlayerFloat(((NetworkBehaviour)Player).NetworkObject, key, (float)(object)value);
			}
		}
		if (typeof(T) == typeof(bool))
		{
