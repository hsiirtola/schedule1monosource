...
public class PunchController : MonoBehaviour
{
	...
	private void Start()
	{
		...
		PlayerSingleton<PlayerInventory>.Instance.onPreItemEquipped.AddListener((UnityAction)delegate
		{
	...
	private void Release()
	{
		...
		Punch(num);
		PlayerSingleton<PlayerMovement>.Instance.SetResidualVelocity(((Component)player).transform.forward, Mathf.Lerp(0f, 300f, num), Mathf.Lerp(0.05f, 0.15f, num));
		if (num >= 1f)
		{
	...
	private void Punch(float power)
	{
		...
		PunchSound.Play();
		player.SendPunch();
		punchRoutine = ((MonoBehaviour)this).StartCoroutine(PunchRoutine());
	...
	private void ExecuteHit(float power)
	{
		...
		if (PlayerSingleton<PlayerCamera>.Instance.LookRaycast(1.25f, out var hit, NetworkSingleton<CombatManager>.Instance.MeleeLayerMask, includeTriggers: true, 0.3f))
		{
			...
			if (componentInParent != null && componentInParent != player)
			{
				...
				Impact impact = new Impact(((RaycastHit)(ref hit)).point, ((Component)PlayerSingleton<PlayerCamera>.Instance).transform.forward, impactForce, impactDamage, EImpactType.Punch, ((NetworkBehaviour)player).NetworkObject, Random.Range(int.MinValue, int.MaxValue));
				...
				Singleton<FXManager>.Instance.CreateImpactFX(impact, componentInParent);
	...
	private void SetPunchingEnabled(bool enabled)
	{
		...
		if (!PunchingEnabled)
		{
			...
			{
				((MonoBehaviour)this).StopCoroutine(punchRoutine);
	...
	{
		if (!PlayerSingleton<PlayerInventory>.InstanceExists || !PlayerSingleton<PlayerCamera>.InstanceExists || (Object)(object)Player.Local == (Object)null || !Singleton<PauseMenu>.InstanceExists)
		{
			return false;
		}
