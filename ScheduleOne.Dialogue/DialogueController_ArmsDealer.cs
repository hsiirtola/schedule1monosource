...
public class DialogueController_ArmsDealer : DialogueController
{
	...
	public class WeaponOption
	{
		...
		public string Name
		{
			get
			{
				...
				return ((BaseItemDefinition)Item).Name;
			}
		}
	...
	public override bool CheckChoice(string choiceLabel, out string invalidReason)
	{
		...
		if (choiceLabel == "GIVE_RDX" && PlayerSingleton<PlayerInventory>.Instance.GetAmountOfItem(((BaseItemDefinition)RDX).ID) == 0)
		{
			invalidReason = "No RDX in inventory";
			return false;
		}
	...
	public override string ModifyDialogueText(string dialogueLabel, string dialogueText)
	{
		...
		if (dialogueLabel == "REQUEST_ACCEPTED")
		{
			questDefeatCartel.EnquireAboutBombEntry.Complete();
		}
		if (dialogueLabel == "BOMB_CREATED")
		{
			TradeRDXForBomb();
		}
	...
	{
		PlayerSingleton<PlayerInventory>.Instance.RemoveAmountOfItem(((BaseItemDefinition)RDX).ID);
		PlayerSingleton<PlayerInventory>.Instance.AddItemToInventory(Bomb.GetDefaultInstance());
	}
}
