...
public class Equippable_MeleeWeapon : Equippable_AvatarViewmodel
{
	...
	private void Release()
	{
		...
		Hit(num);
		PlayerSingleton<PlayerMovement>.Instance.SetResidualVelocity(((Component)Player.Local).transform.forward, Mathf.Lerp(0f, 300f, num), Mathf.Lerp(0.05f, 0.15f, num));
		if (num >= 1f)
		{
	...
	private void Hit(float power)
	{
		...
		WhooshSound.Play();
		hitRoutine = ((MonoBehaviour)this).StartCoroutine(HitRoutine());
		IEnumerator HitRoutine()
		{
			yield return (object)new WaitForSeconds(Mathf.Lerp(MinHitDelay, MaxHitDelay, power));
			ExecuteHit(power);
			IsAttacking = false;
	...
	private void ExecuteHit(float power)
	{
		...
		if (componentInParent != null)
		{
			...
			Impact impact = new Impact(((RaycastHit)(ref hit)).point, ((Component)PlayerSingleton<PlayerCamera>.Instance).transform.forward, impactForce, impactDamage, ImpactType, ((NetworkBehaviour)Player.Local).NetworkObject, Random.Range(int.MinValue, int.MaxValue));
