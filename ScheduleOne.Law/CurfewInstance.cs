...
public class CurfewInstance
{
	...
	{
		if (!Enabled && Singleton<LawController>.Instance.LE_Intensity >= IntensityRequirement && (NetworkSingleton<TimeManager>.Instance.IsSleepInProgress || ignoreSleepReq))
		{
			Enable();
		}
	}
	...
	private void MinPass()
	{
		if (Enabled)
		{
			...
			if (shouldDisable && NetworkSingleton<TimeManager>.Instance.IsSleepInProgress)
			{
				Disable();
			}
		}
