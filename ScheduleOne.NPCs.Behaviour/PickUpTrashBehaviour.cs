...
public class PickUpTrashBehaviour : Behaviour
{
	...
	{
		if ((Object)(object)base.Npc.Avatar.CurrentEquippable == (Object)null || base.Npc.Avatar.CurrentEquippable.AssetPath != "Tools/TrashGrabber/TrashGrabber_AvatarEquippable")
		{
	...
	public override void OnActiveTick()
	{
		...
		if (base.Npc.Movement.IsMoving)
		{
			if (Cleaner.DEBUG)
			{
				Console.Log("Waiting for movement to finish");
			}
		}
		else if (actionCoroutine != null)
		{
			if (Cleaner.DEBUG)
			{
		...
		else if (!AreActionConditionsMet(checkAccess: false))
		{
	...
	private void GoToTarget()
	{
		...
		{
			SetDestination(((Component)TargetTrash).transform.position);
		}
	}
	...
	private bool IsAtDestination()
	{
		...
		return Vector3.Distance(((Component)base.Npc).transform.position, ((Component)TargetTrash).transform.position) <= 2f;
	}

	private bool AreActionConditionsMet(bool checkAccess)
	{
		//IL_003d: Unknown result type (might be due to invalid IL or missing references)
		if ((Object)(object)TargetTrash == (Object)null)
		{
			return false;
		}
		if (TargetTrash.Draggable.IsBeingDragged)
		{
			return false;
		}
		if (checkAccess && !base.Npc.Movement.CanGetTo(((Component)TargetTrash).transform.position, 2f))
		{
			return false;
		}
		return true;
	...
	public override void NetworkInitialize___Early()
	{
		...
		if (!NetworkInitialize___EarlyScheduleOne_002ENPCs_002EBehaviour_002EPickUpTrashBehaviourAssembly_002DCSharp_002Edll_Excuted)
		{
			NetworkInitialize___EarlyScheduleOne_002ENPCs_002EBehaviour_002EPickUpTrashBehaviourAssembly_002DCSharp_002Edll_Excuted = true;
	...
	{
		if (!NetworkInitialize__LateScheduleOne_002ENPCs_002EBehaviour_002EPickUpTrashBehaviourAssembly_002DCSharp_002Edll_Excuted)
		{
			NetworkInitialize__LateScheduleOne_002ENPCs_002EBehaviour_002EPickUpTrashBehaviourAssembly_002DCSharp_002Edll_Excuted = true;
	...
	private void RpcLogic___PerformAction_2166136261()
	{
		...
		IEnumerator Action()
		{
			...
			else
			{
				if (InstanceFinder.IsServer)
				{
					base.Npc.Movement.FacePoint(((Component)TargetTrash).transform.position);
				}
				...
				if (InstanceFinder.IsServer)
				{
					Cleaner.trashGrabberInstance.AddTrash(TargetTrash.ID, 1);
					if ((Object)(object)TargetTrash != (Object)null)
					{
						TargetTrash.DestroyTrash();
					}
				}
