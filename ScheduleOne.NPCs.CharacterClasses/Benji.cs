...
public class Benji : Dealer
{
	...
	protected override void OnTick()
	{
		...
		{
			if (Inventory.ItemSlots[i].Quantity != 0 && Inventory.ItemSlots[i].ItemInstance is WeedInstance)
			{
				num += (Inventory.ItemSlots[i].ItemInstance as WeedInstance).Amount * Inventory.ItemSlots[i].Quantity;
			}
		}
	...
	protected override void AddCustomer(Customer customer)
	{
		...
		NetworkSingleton<VariableDatabase>.Instance.SetVariableValue("Benji_CustomerCount", base.AssignedCustomers.Count.ToString());
	}
	...
	protected override void UpdatePotentialDealerPoI()
	{
		...
		((Behaviour)base.PotentialDealerPoI).enabled = false;
	}
