...
public class NPCSignal : NPCAction
{
	...
	{
		return TimeManager.Get12HourTime(StartTime);
	}

	public override int GetEndTime()
	{
		return TimeManager.AddMinutesTo24HourTime(StartTime, MaxDuration);
	}
	...
	{
		base.MinPassed();
		if (StartedThisCycle && !NetworkSingleton<TimeManager>.Instance.IsCurrentTimeWithinRange(StartTime, GetEndTime()))
		{
			StartedThisCycle = false;
		}
	}
