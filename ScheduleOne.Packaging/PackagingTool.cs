...
public class PackagingTool : MonoBehaviour
{
	public class PackagingInstance
	{
		...
		public void ChangePosition(float angleDelta)
		{
			...
			Quaternion val = Quaternion.Euler(0f, 0f - AnglePosition, 0f);
			Quaternion val2 = Container.parent.rotation * val;
			ContainerRb.MoveRotation(val2);
		}
	}
	...
	public void Deinitialize()
	{
		...
		{
			Object.Destroy((Object)(object)((Component)ProductInstances[i]).gameObject);
		}
		ProductInstances.Clear();
		for (int j = 0; j < PackagingInstances.Count; j++)
		{
			Object.Destroy((Object)(object)((Component)PackagingInstances[j].Container).gameObject);
		}
		PackagingInstances.Clear();
		for (int k = 0; k < FinalizedPackaging.Count; k++)
		{
			Object.Destroy((Object)(object)((Component)FinalizedPackaging[k]).gameObject);
		}
		FinalizedPackaging.Clear();
		if (finalizeCoroutine != null)
		{
			((MonoBehaviour)this).StopCoroutine(finalizeCoroutine);
	...
	{
		((TMP_Text)ProductCountText).text = ProductInHopper.ToString();
		((Component)ProductCountText).gameObject.SetActive(ProductInHopper > 0);
	}

	private void UpdateConveyor()
	{
		float num = Mathf.MoveTowards(conveyorVelocity, (float)directionInput, ConveyorAcceleration * Time.deltaTime);
	...
	private void Finalize(PackagingInstance instance)
	{
		...
		finalizeCoroutine = ((MonoBehaviour)this).StartCoroutine(FinalizeRoutine());
		IEnumerator FinalizeRoutine()
		{
			...
			KickSound.Play();
			yield return (object)new WaitForSeconds(0.05f);
			((Component)instance.Packaging).transform.SetParent(ProductContainer);
			instance.Packaging.Rb.isKinematic = false;
			instance.Packaging.NormalRBDrag = 0f;
			instance.Packaging.Rb.AddForceAtPosition(KickOrigin.forward * KickForce, KickOrigin.position, (ForceMode)2);
			instance.Packaging.Rb.AddTorque(Random.insideUnitSphere * KickForce, (ForceMode)2);
			Station.PackSingleInstance();
			Object.Destroy((Object)(object)((Component)instance.Container).gameObject);
	...
	private void DropProduct()
	{
		...
		if (ProductInHopper > 0)
		{
			...
			UpdateScreen();
			DropSound.Play();
			FunctionalProduct functionalProduct = Object.Instantiate<FunctionalProduct>(ProductPrefab, HopperDropPoint.position, HopperDropPoint.rotation);
			functionalProduct.Initialize(ProductItem);
			((Component)functionalProduct).transform.SetParent(ProductContainer);
			functionalProduct.ClampZ = true;
			functionalProduct.DragProjectionMode = Draggable.EDragProjectionMode.FlatCameraForward;
			functionalProduct.Rb.collisionDetectionMode = (CollisionDetectionMode)2;
			functionalProduct.Rb.AddForce(Vector3.down * ProductInitialForce, (ForceMode)2);
			functionalProduct.Rb.AddTorque(Random.insideUnitSphere * ProductRandomTorque, (ForceMode)2);
	...
	private void CheckInsertions()
	{
		...
		for (int i = 0; i < ProductInstances.Count; i++)
		{
			if (!((Object)(object)ProductInstances[i].Rb == (Object)null) && !ProductInstances[i].Rb.isKinematic)
			{
				Bounds bounds = ((Collider)HopperInputCollider).bounds;
				if (((Bounds)(ref bounds)).Contains(((Component)ProductInstances[i]).transform.position))
				{
	...
	private void InsertIntoHopper(FunctionalProduct product)
	{
		...
		Object.Destroy((Object)(object)((Component)product).gameObject);
	...
	private void DeployPackaging()
	{
		...
		{
			ConcealedPackaging--;
			GameObject val = new GameObject("Packaging Container");
			val.transform.SetParent(PackagingContainer);
			val.transform.localPosition = Vector3.zero;
			val.transform.localRotation = Quaternion.identity;
			FunctionalPackaging functionalPackaging = Object.Instantiate<FunctionalPackaging>(PackagingPrefab, val.transform);
			functionalPackaging.AutoEnableSealing = false;
			functionalPackaging.Initialize(Station, null, align: false);
			functionalPackaging.Rb.collisionDetectionMode = (CollisionDetectionMode)2;
			if (functionalPackaging is FunctionalBaggie)
			{
				((Component)functionalPackaging).transform.position = BaggieStartPoint.position;
				functionalPackaging.Rb.position = BaggieStartPoint.position;
				((Component)functionalPackaging).transform.rotation = BaggieStartPoint.rotation;
				functionalPackaging.Rb.rotation = BaggieStartPoint.rotation;
			}
			else if (functionalPackaging is FunctionalJar)
			{
				((Component)functionalPackaging).transform.position = JarStartPoint.position;
				functionalPackaging.Rb.position = JarStartPoint.position;
				((Component)functionalPackaging).transform.rotation = JarStartPoint.rotation;
			...
			packagingInstance.Container = val.transform;
			packagingInstance.ContainerRb = val.AddComponent<Rigidbody>();
			packagingInstance.ContainerRb.isKinematic = true;
			packagingInstance.ContainerRb.useGravity = false;
			packagingInstance.ContainerRb.collisionDetectionMode = (CollisionDetectionMode)2;
