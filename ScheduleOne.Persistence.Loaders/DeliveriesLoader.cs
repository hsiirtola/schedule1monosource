...
public class DeliveriesLoader : Loader
{
	public override void Load(string mainPath)
	{
		...
		if (TryLoadFile(Path.Combine(mainPath, "Deliveries"), out var contents) || TryLoadFile(mainPath, out contents))
		{
			...
			NetworkSingleton<DeliveryManager>.Instance.Load(deliveriesData);
			if (deliveriesData != null && deliveriesData.ActiveDeliveries != null && deliveriesData.DeliveryVehicles != null)
			{
				flag = true;
