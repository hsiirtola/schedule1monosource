...
public class ProductManagerLoader : Loader
{
	public override void Load(string mainPath)
	{
		...
		if (Directory.Exists(text) && !flag)
		{
			...
			ShroomProductLoader shroomProductLoader = new ShroomProductLoader();
			...
			for (int i = 0; i < files.Length; i++)
			{
				...
				if (string.IsNullOrEmpty(productData.Name))
				{
					...
					{
						productData.Name = "Product " + Random.Range(0, 1000);
					}
					flag2 = true;
				}
				...
				case EDrugType.Shrooms:
					shroomProductLoader.Load(files[i]);
					break;
				default:
			...
			if (productManagerData.ProductPrices != null)
			{
				for (int n = 0; n < productManagerData.ProductPrices.Length; n++)
				{
					if (productManagerData.ProductPrices[n] != null)
					{
						...
						{
							NetworkSingleton<ProductManager>.Instance.SetPrice(null, ((BaseItemDefinition)item).ID, stringIntPair.Int);
						}
					}
				}
			}
			NetworkSingleton<ProductManager>.Instance.ContractReceipts = productManagerData.ContractReceipts?.ToList() ?? new List<ContractReceipt>();
			Console.Log("Loaded " + NetworkSingleton<ProductManager>.Instance.ContractReceipts.Count + " contract receipts.");
		}
		else
		{
	...
	private void SanitizeProductData(ProductData data)
	{
		...
		if (string.IsNullOrEmpty(data.Name))
		{
			...
			{
				data.Name = "Product " + Random.Range(0, 1000);
			}
		}
	...
	private void LoadProducts(ProductManagerData productData)
	{
		...
		if (productData.CreatedShrooms == null)
		{
			return;
		}
		ShroomProductData[] createdShrooms = productData.CreatedShrooms;
		foreach (ShroomProductData shroomProductData in createdShrooms)
		{
			if (shroomProductData != null)
			{
				SanitizeProductData(shroomProductData);
