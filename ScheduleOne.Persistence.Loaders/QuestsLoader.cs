	...
	public override void Load(string mainPath)
	{
		if (TryLoadFile(mainPath, out var contents))
		{
			...
			foreach (ContractData contractData in contracts)
			{
				if (contractData != null)
				{
					...
					{
						NetworkSingleton<QuestManager>.Instance.CreateContract_Local(contractData.Title, contractData.Description, contractData.Entries, contractData.GUID, contractData.IsTracked, ((Component)nPC).GetComponent<Customer>(), contractData.Payment, contractData.ProductList, contractData.DeliveryLocationGUID, contractData.DeliveryWindow, contractData.Expires, new GameDateTime(contractData.ExpiryDate), contractData.PickupScheduleIndex, new GameDateTime(contractData.AcceptTime));
					}
				}
			}
		}
		else
		{
			...
			for (int j = 0; j < files.Length; j++)
			{
				...
				{
					Debug.LogError((object)("Error loading quest data: " + ex2.Message));
				}
				if (questData2 == null)
				{
					continue;
				...
				if (questData2.DataType == "DeaddropQuestData")
				{
					...
					{
						Debug.LogError((object)("Error loading quest data: " + ex3.Message));
					}
					...
					if ((Object)(object)deadDrop2 == (Object)null)
					{
				...
				if ((Object)(object)quest2 == (Object)null)
				{
			...
			for (int k = 0; k < files2.Length; k++)
			{
				...
				{
					Debug.LogError((object)("Error loading contract data: " + ex4.Message));
				}
				if (contractData2 != null)
				{
					NPC nPC2 = GUIDManager.GetObject<NPC>(new Guid(contractData2.CustomerGUID));
					if ((Object)(object)nPC2 == (Object)null)
					{
						Console.LogWarning("Failed to find customer with GUID: " + contractData2.CustomerGUID);
					}
					else
					{
						NetworkSingleton<QuestManager>.Instance.CreateContract_Local(contractData2.Title, contractData2.Description, contractData2.Entries, contractData2.GUID, contractData2.IsTracked, ((Component)nPC2).GetComponent<Customer>(), contractData2.Payment, contractData2.ProductList, contractData2.DeliveryLocationGUID, contractData2.DeliveryWindow, contractData2.Expires, new GameDateTime(contractData2.ExpiryDate), contractData2.PickupScheduleIndex, new GameDateTime(contractData2.AcceptTime));
					}
				}
			}
		}
