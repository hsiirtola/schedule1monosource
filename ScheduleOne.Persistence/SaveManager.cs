...
public class SaveManager : PersistentSingleton<SaveManager>
{
	...
	{
		base.Awake();
		if ((Object)(object)Singleton<SaveManager>.Instance == (Object)null || (Object)(object)Singleton<SaveManager>.Instance != (Object)(object)this)
		{
			return;
		}
	...
	public void CheckSaveFolderInitialized()
	{
		...
		{
			string path = ((object)SteamUser.GetSteamID()/*cast due to .constrained prefix*/).ToString();
		...
		{
			Console.LogWarning("Steamworks not intialized in time for SaveManager! Using save container path: " + IndividualSavesContainerPath);
		}
		if (HasWritePermissionOnDir(IndividualSavesContainerPath))
		{
			...
			if ((Object)(object)WriteIssueDisplay != (Object)null)
			{
				((Component)WriteIssueDisplay).gameObject.SetActive(false);
			}
		}
		else
		{
			...
			if ((Object)(object)WriteIssueDisplay != (Object)null)
			{
				((Component)WriteIssueDisplay).gameObject.SetActive(true);
			}
		}
	}
	...
	private void Update()
	{
		if (Singleton<LoadManager>.Instance.IsGameLoaded && Singleton<LoadManager>.Instance.LoadedGameFolderPath != string.Empty && Input.GetKeyDown((KeyCode)286) && (Application.isEditor || Debug.isDebugBuild))
		{
			Save();
		}
	...
	public void Save(string saveFolderPath)
	{
		...
		SaveError = false;
		((MonoBehaviour)this).StartCoroutine(SaveRoutine());
	...
	public void CreateSaveBackup(SaveInfo saveInfo)
	{
		if (saveInfo == null)
		{
			Console.LogError("No save info provided for backup");
			return;
		}
		try
		{
			...
			string fileName = saveInfo.OrganisationName + " (" + saveInfo.SaveVersion + ", save slot " + saveInfo.SaveSlotNumber + ")";
			fileName = MakeFileSafe(fileName);
			string text = Path.Combine(BackupFolderPath, fileName + ".zip");
			SaveExportButton.ZipSaveFolder(saveInfo.SavePath, text);
			Console.Log("Created save backup at: " + text);
		}
		catch (Exception ex)
		{
			Console.LogError("Failed to create save backup: " + ex);
		}
	}
