...
public class PlayerManager : Singleton<PlayerManager>, IBaseSaveable, ISaveable
{
	...
	public void LoadPlayer(PlayerData data, string containerPath)
	{
		...
		if ((Object)(object)player == (Object)null && InstanceFinder.IsServer)
		{
	...
	public void AllPlayerFilesLoaded()
	{
		...
		if (InstanceFinder.IsServer)
		{
			...
			{
				text = ((object)SteamUser.GetSteamID()/*cast due to .constrained prefix*/).ToString();
			}
	...
	public bool TryGetPlayerData(string playerCode, out PlayerData data, out string inventoryString, out string appearanceString, out string clothingString, out VariableData[] variables)
	{
		...
		if (data != null)
		{
			...
			if (playerLoader.TryLoadFile(text, "Clothing", out var contents3))
			{
				clothingString = contents3;
			}
			else
			{
				Console.LogWarning("Failed to load player clothing under " + text);
			}
			bool flag = false;
			if (playerLoader.TryLoadFile(text, "Variables", out var contents4))
			{
				...
				{
					flag = true;
					VariableData[] variables2 = variableCollectionData.Variables;
					foreach (VariableData variableData in variables2)
					{
			...
			if (!flag)
			{
				string text2 = Path.Combine(text, "Variables");
				if (Directory.Exists(text2))
				{
					Console.Log("Loading legacy player variables from " + text2);
					...
					{
						if (variablesLoader.TryLoadFile(files[num2], out var contents5, autoAddExtension: false))
						{
							VariableData item = null;
							try
							{
								item = JsonUtility.FromJson<VariableData>(contents5);
							}
							catch (Exception ex2)
							{
				...
				{
					Console.LogWarning("Failed to load player variables under " + text);
				}
			}
		}
		if (list.Count > 0)
