...
public class LabOvenSolidTask : Task
{
	...
	public LabOvenSolidTask(LabOven oven)
	{
		...
		{
			stationDraggables[i] = ((Component)stationItems[i]).GetComponentInChildren<Draggable>();
		}
	...
	public override void Success()
	{
		...
		Oven.SendCookOperation(new OvenCookOperation(((BaseItemInstance)ingredient).ID, ingredientQuality, ingredientQuantity, iD));
		base.Success();
	}
	...
	private void ProgressStep()
	{
		...
		if (CurrentStep == EStep.CloseDoor)
		{
			...
			for (int i = 0; i < stationDraggables.Length; i++)
			{
				...
				Object.Destroy((Object)(object)stationDraggables[i].Rb);
				((Component)stationItems[i]).transform.SetParent(Oven.SquareTray);
			}
		}
		if (CurrentStep == EStep.PressButton)
		{
	...
	private void CheckStep_PlaceItems()
	{
		...
		for (int i = 0; i < stationDraggables.Length; i++)
		{
			if (stationDraggables[i].IsHeld)
			{
				return;
			}
			Vector3 velocity = stationDraggables[i].Rb.velocity;
			if (((Vector3)(ref velocity)).magnitude > 0.02f)
			{
				return;
			}
			Bounds bounds = ((Collider)Oven.TrayDetectionArea).bounds;
			if (!((Bounds)(ref bounds)).Contains(((Component)stationDraggables[i]).transform.position))
			{
				return;
			}
		}
