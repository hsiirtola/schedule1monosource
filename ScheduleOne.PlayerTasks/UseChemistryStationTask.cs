...
public class UseChemistryStationTask : Task
{
	...
	public UseChemistryStationTask(ChemistryStation station, StationRecipe recipe)
	{
		...
		((Component)station.StaticBeaker).gameObject.SetActive(false);
		...
		for (int i = 0; i < recipe.Ingredients.Count; i++)
		{
			...
			foreach (ItemDefinition item in recipe.Ingredients[i].Items)
			{
				...
				{
					if (station.IngredientSlots[j].ItemInstance != null && ((BaseItemDefinition)station.IngredientSlots[j].ItemInstance.Definition).ID == ((BaseItemDefinition)storableItemDefinition2).ID)
					{
			...
			((Component)stationItem).transform.position = station.IngredientTransforms[i].position;
			stationItem.Initialize(storableItemDefinition);
			((Component)stationItem).transform.rotation = station.IngredientTransforms[i].rotation;
			if (stationItem.HasModule<IngredientModule>())
			{
				...
				foreach (IngredientPiece ingredientPiece in pieces)
				{
					...
					((Component)ingredientPiece).GetComponent<Draggable>().CanBeMultiDragged = true;
				}
			}
			else if (stationItem.HasModule<PourableModule>())
			{
				stationItem.ActivateModule<PourableModule>();
				Draggable componentInChildren = ((Component)stationItem).GetComponentInChildren<Draggable>();
	...
	private void ProgressStep()
	{
		...
		if (CurrentStep == ChemistryStation.EStep.Stir)
		{
			...
			((Component)Station.StaticStirringRod).gameObject.SetActive(false);
		}
		if (CurrentStep == ChemistryStation.EStep.LowerBoilingFlask)
		{
			if ((Object)(object)stirringRod != (Object)null)
			{
				stirringRod.Destroy();
			}
		...
		if (CurrentStep == ChemistryStation.EStep.PourIntoBoilingFlask)
		{
			...
			((Behaviour)beaker.Fillable).enabled = false;
	...
	private void CheckStep_CombineIngredients()
	{
		...
		for (int i = 0; i < items.Count; i++)
		{
			...
			else
			{
				...
				{
					if ((Object)(object)pieces[j].CurrentLiquidContainer == (Object)null)
					{
						flag = false;
						break;
					}
	...
	{
		float num = stirringRod.CurrentStirringSpeed * Time.deltaTime / 1.5f;
		if (num > 0f)
		{
	...
	private void CheckStep_StartHeat()
	{
		...
		if (Station.BoilingFlask.OverheatScale >= 1f)
		{
			Fail();
			NetworkSingleton<CombatManager>.Instance.CreateExplosion(((Component)Station.ExplosionPoint).transform.position, ExplosionData.DefaultSmall);
			Player.Local.Health.TakeDamage(100f);
		}
	...
	{
		base.StopTask();
		if (Outcome != EOutcome.Success && Outcome != EOutcome.Fail)
		{
		...
		beaker.Destroy();
		((Component)Station.StaticBeaker).gameObject.SetActive(true);
		((Component)Station.StaticFunnel).gameObject.SetActive(true);
		((Component)Station.StaticStirringRod).gameObject.SetActive(true);
		Station.LabStand.SetPosition(1f);
		Station.LabStand.SetInteractable(e: false);
		Station.Burner.SetInteractable(e: false);
		Station.BoilingFlask.SetCanvasVisible(visible: false);
