...
public class Quest_TheDeepEnd : Quest
{
	...
	protected override void Start()
	{
		...
		instance2.onSleepStart = (Action)Delegate.Combine(instance2.onSleepStart, new Action(BeforeSleep));
		Singleton<SleepCanvas>.Instance.onSleepEndFade.AddListener(new UnityAction(SleepFadeOut));
	}
	...
	public void SetupFirstMeeting()
	{
		...
		Switch.SwitchOn();
		Thomas.DialogueHandler.onDialogueNodeDisplayed.AddListener((UnityAction<string>)ThomasDialogueNodeDisplayed);
	}
	...
	private void ThomasDialogueNodeDisplayed(string nodeLabel)
	{
		...
		{
			Debug.Log((object)"Intro meeting done!");
		...
		IEnumerator Wait()
		{
			yield return (object)new WaitUntil((Func<bool>)(() => (Object)(object)Player.Local.CurrentProperty == (Object)null));
	...
	{
		if (InstanceFinder.IsServer && Quest.GetQuest("Sink or Swim").State == EQuestState.Completed)
		{
	...
	private void SleepFadeOut()
	{
		...
		if (kidnapQueued)
		{
			...
			((Component)Player.Local).transform.forward = MeetingTeleportPoint.forward;
		}
	}
