...
public class StorageVisualizer : MonoBehaviour
{
	...
	public void AddSlot(ItemSlot slot, bool update = false)
	{
		...
		{
			itemSlots.Add(slot);
			slot.onItemDataChanged = (Action)Delegate.Combine(slot.onItemDataChanged, new Action(QueueRefresh));
		}
		if (update)
		{
			QueueRefresh();
	...
	public virtual void RefreshVisuals()
	{
		...
		if (num > 0 || num == 0 || FullRefreshOnItemRemoved)
		{
			...
			foreach (StorableItemInstance item2 in list)
			{
				...
				_ = list4[0].FootprintX;
				_ = list4[0].FootprintY;
				...
				foreach (StoredItem item3 in list4)
				{
					...
					{
						if (StorageGrids[l].TryFitItem(item3.FootprintX, item3.FootprintY, new List<Coordinate>(), out var originCoordinate, out var rotation))
						{
		...
		for (int m = 0; m < list3.Count; m++)
		{
			...
			{
				Object.Destroy((Object)(object)((Component)list3[m]).gameObject);
			}
		}
	}
	...
	private List<StoredItem> EnsureSufficientStoredItems(StorableItemInstance item, int quantityRequirement)
	{
		...
		if (num < quantityRequirement)
		{
			...
			{
				StoredItem component = ((Component)Object.Instantiate<StoredItem>(item.StoredItem, ((Object)(object)ItemContainer != (Object)null) ? ItemContainer : ((Component)this).transform)).GetComponent<StoredItem>();
	...
	private void DestroyExcessStoredItems(StorableItemInstance item, int quantityRequirement)
	{
		...
		if (num > quantityRequirement)
		{
			...
			for (int i = 0; i < num2; i++)
			{
				activeStoredItems[item][activeStoredItems[item].Count - 1].Destroy();
