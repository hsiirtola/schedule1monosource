	...
	public void Initialize(int _slotToOverwrite, MainMenuScreen previousScreen)
	{
		...
		if (directories.Length != 0)
		{
			...
			if (LoadManager.TryLoadSaveInfo(text, -1, out var saveInfo, requireGameFile: true))
			{
				...
				flag = true;
				((TMP_Text)OrganisationNameLabel).text = saveInfo.OrganisationName;
				((TMP_Text)NetworthLabel).text = MoneyManager.FormatAmount(saveInfo.Networth);
				((TMP_Text)VersionLabel).text = "v" + saveInfo.SaveVersion;
				if (LoadManager.SaveGames[slotToOverwrite] != null)
				{
					((TMP_Text)WarningLabel).text = "Warning: This will overwrite the current save in slot " + (slotToOverwrite + 1) + " (" + LoadManager.SaveGames[slotToOverwrite].OrganisationName + ").";
		...
		((Selectable)ConfirmButton).interactable = flag;
