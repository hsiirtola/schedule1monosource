...
public class SetupScreen : MainMenuScreen
{
	...
	{
		((UnityEvent<string>)(object)((TMP_InputField)InputField).onSubmit).AddListener((UnityAction<string>)delegate
		{
			StartGame();
		});
		((Component)SkipIntroContainer).gameObject.SetActive(true);
	}
	...
	private void Update()
	{
		if (base.IsOpen)
		{
			((Selectable)StartButton).interactable = IsInputValid() && Singleton<Lobby>.Instance.IsHost;
			((Component)NotHostWarning).gameObject.SetActive(!Singleton<Lobby>.Instance.IsHost);
		}
	}
	...
	public void StartGame()
	{
		...
		string text = Path.Combine(Singleton<SaveManager>.Instance.IndividualSavesContainerPath, "SaveGame_" + (slotIndex + 1));
		...
		string json = new GameData(seed: Random.Range(0, int.MaxValue), organisationName: ((TMP_InputField)InputField).text, settings: new GameSettings()).GetJson();
		...
		Singleton<LoadManager>.Instance.StartGame(LoadManager.SaveGames[slotIndex], allowLoadStacking: false, allowSaveBackup: false);
	}

	private bool IsInputValid()
	{
		return !string.IsNullOrEmpty(((TMP_InputField)InputField).text);
	}
	...
	{
		CopyFilesRecursively(Path.Combine(Application.streamingAssetsPath, "DefaultSave"), folderPath);
	}
