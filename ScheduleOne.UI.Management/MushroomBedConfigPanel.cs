...
public class MushroomBedConfigPanel : ConfigPanel
{
	...
	protected override void BindInternal(List<EntityConfiguration> configs)
	{
		...
		foreach (MushroomBedConfiguration config in configs)
		{
			if (config == null)
			{
				Console.LogError("Failed to cast EntityConfiguration to MushroomBedConfiguration");
				return;
			}
			list.Add(config.Spawn);
