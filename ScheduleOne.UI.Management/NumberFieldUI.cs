...
public class NumberFieldUI : MonoBehaviour
{
	...
	public void Bind(List<NumberField> field)
	{
		...
		Fields[Fields.Count - 1].onItemChanged.AddListener((UnityAction<float>)Refresh);
		((TMP_Text)MinValueLabel).text = Fields[0].MinValue.ToString();
		((TMP_Text)MaxValueLabel).text = Fields[0].MaxValue.ToString();
	...
	private void Refresh(float newVal)
	{
		if (AreFieldsUniform())
		{
			((TMP_Text)ValueLabel).text = newVal.ToString();
		}
		else
		{
			((TMP_Text)ValueLabel).text = "#";
		}
