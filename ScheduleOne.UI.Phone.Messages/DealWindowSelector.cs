...
public class DealWindowSelector : MonoBehaviour
{
	...
	private void Start()
	{
		...
		{
			button.OnSelected.AddListener((UnityAction)delegate
			{
	...
	private void UpdateTime()
	{
		...
		float normalizedTimeOfDay = NetworkSingleton<TimeManager>.Instance.NormalizedTimeOfDay;
		((Transform)CurrentTimeArm).localRotation = Quaternion.Euler(0f, 0f, Mathf.Lerp(0f, -360f, normalizedTimeOfDay));
	}
	...
	private void UpdateWindowValidity()
	{
		...
		int dailyMinSum = NetworkSingleton<TimeManager>.Instance.DailyMinSum;
		WindowSelectorButton[] array = buttons;
		foreach (WindowSelectorButton obj in array)
		{
			...
			bool interactable = ((Selectable)obj.Button).interactable;
			bool flag = num - dailyMinSum > 120;
			obj.SetInteractable(flag);
			if (!flag && interactable != flag)
			{
