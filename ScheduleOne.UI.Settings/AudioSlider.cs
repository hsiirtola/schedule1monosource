...
public class AudioSlider : SettingsSlider
{
	...
	protected virtual void OnEnable()
	{
		if (Master)
		{
			SetValueWithoutNotify(Singleton<AudioManager>.Instance.MasterVolume / 0.01f);
		}
		else
		{
			SetValueWithoutNotify(Singleton<AudioManager>.Instance.GetVolume(AudioType, scaled: false) / 0.01f);
		}
	}
	...
	protected override void OnValueChanged(float value)
	{
		...
		else
		{
			...
			case EAudioType.Weather:
				Singleton<ScheduleOne.DevUtilities.Settings>.Instance.AudioSettings.WeatherVolume = value * 0.01f;
				break;
			}
		}
