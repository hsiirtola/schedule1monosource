...
public class ClothingShopInterface : ShopInterface
{
	...
	{
		base.Start();
		ColorPicker.onColorPicked.AddListener((UnityAction<EClothingColor>)ColorPicked);
	}

	public override void ListingClicked(ListingUI listingUI)
	{
		if (listingUI.Listing.Item.IsUnlocked)
		{
	...
	protected override void Exit(ExitAction action)
	{
		if (!action.Used)
		{
			if ((Object)(object)ColorPicker != (Object)null && ColorPicker.IsOpen)
			{
				action.Used = true;
				ColorPicker.Close();
			}
	...
	public override bool HandoverItems()
	{
		...
		for (int i = 0; i < list.Count; i++)
		{
			...
			if (num > 0)
			{
				Debug.LogWarning((object)("Failed to handover all items in cart: " + ((BaseItemInstance)clothingInstance).Name));
				result = false;
			}
		}
		return result;
