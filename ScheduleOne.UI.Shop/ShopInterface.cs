...
public class ShopInterface : MonoBehaviour, ISaveable
{
	...
	public string ShopDescription = "This is a shop.";
	...
	[SerializeField]
	private int addToCartTier1Amount = 1;

	[SerializeField]
	private int addToCartTier2Amount = 5;

	[SerializeField]
	private int addToCartTier3Amount = 20;

	[Header("Modify Cart Popup Values")]
	public int minModifyAmount = 1;

	public int modifyTier1Amount = 1;

	public int modifyTier2Amount = 5;

	public int modifyTier3Amount = 20;
	...
	protected virtual void Awake()
	{
		...
		Listings = Listings.OrderBy((ShopListing x) => ((BaseItemDefinition)x.Item).Name).ToList();
		categoryButtons = ((Component)this).GetComponentsInChildren<CategoryButton>().ToList();
		((TMP_Text)StoreNameLabel).text = ShopName;
		ListingContainer.anchoredPosition = Vector2.zero;
		AmountSelector.onSubmitted.AddListener((UnityAction<int>)QuantitySelected);
		AllShops.Add(this);
	}
	...
	protected virtual void Start()
	{
		...
		foreach (ShopListing listing in Listings)
		{
			...
			{
				Debug.LogWarning((object)("Null listing or item in shop: " + ShopName));
				continue;
			}
		...
		instance.onSleepStart = (Action)Delegate.Remove(instance.onSleepStart, new Action(OnDayPass));
		TimeManager instance2 = NetworkSingleton<TimeManager>.Instance;
		instance2.onSleepStart = (Action)Delegate.Combine(instance2.onSleepStart, new Action(OnDayPass));
		TimeManager instance3 = NetworkSingleton<TimeManager>.Instance;
		instance3.onWeekPass = (Action)Delegate.Remove(instance3.onWeekPass, new Action(OnWeekPass));
		TimeManager instance4 = NetworkSingleton<TimeManager>.Instance;
		instance4.onWeekPass = (Action)Delegate.Combine(instance4.onWeekPass, new Action(OnWeekPass));
		InitializeSaveable();
	}
	...
	private void OnValidate()
	{
		...
		{
			if (!((Object)(object)Listings[i].Item == (Object)null))
			{
				string text = "(";
				...
				text += ")";
				Listings[i].name = ((BaseItemDefinition)Listings[i].Item).Name + " ($" + Listings[i].Price + ") " + text;
				if (Listings[i].Item.RequiresLevelToPurchase)
				{
					...
					shopListing.name = name + " [Rank " + requiredRank.ToString() + "]";
				}
			}
		}
	}
	...
	protected virtual void Update()
	{
		...
		{
			if (amountSelectorMouseUp)
			{
				AmountSelector.Close();
				selectedListing = null;
			}
			amountSelectorMouseUp = true;
		}
	}
	...
	public virtual void SetIsOpen(bool isOpen)
	{
		...
		IsOpen = isOpen;
		listingPanel.DeselectSelectable();
		...
		if (isOpen)
		{
			...
			if (ShowCurrencyHint)
			{
				...
				((MonoBehaviour)this).Invoke("Hint", 10.5f);
			}
			Singleton<UIScreenManager>.Instance.AddScreen(shopScreen, Exit);
		}
		else
		{
			...
			DetailPanel.Close();
			AmountSelector.Close();
			Singleton<UIScreenManager>.Instance.RemoveScreen(shopScreen);
		}
		((Behaviour)Canvas).enabled = isOpen;
		((Component)Container).gameObject.SetActive(isOpen);
	}
	...
	{
		if (!action.Used && action.exitType != ExitType.RightClick && IsOpen)
		{
			action.Used = true;
			SetIsOpen(isOpen: false);
		}
	...
	{
		ListingUI component = Object.Instantiate<GameObject>(((Component)ListingUIPrefab).gameObject, (Transform)(object)ListingContainer).GetComponent<ListingUI>();
		...
		listingPanel.AddSelectable(((Component)component).GetComponent<UISelectable>());
	}
	...
	public void SelectCategory(EShopCategory category)
	{
		...
		if ((Object)(object)categoryButton == (Object)null)
		{
	...
	{
		if (listingUI.Listing.Item.IsUnlocked && listingUI.CanAddToCart())
		{
			int quantity = 1;
			if (AmountSelector.IsOpen)
			{
				quantity = Mathf.Clamp(AmountSelector.SelectedAmount, 1, listingUI.Listing.IsUnlimitedStock ? 100000000 : listingUI.Listing.CurrentStockMinusCart);
			}
	...
	private void PullStockVariables()
	{
		for (int i = 0; i < Listings.Count; i++)
		{
			if (Listings[i].TieStockToNumberVariable)
			{
				Listings[i].PullStockFromVariable();
			}
		}
	}
	...
	private void RefreshShownItems()
	{
		for (int i = 0; i < listingUI.Count; i++)
		{
			...
			{
				((Component)listingUI[i]).gameObject.SetActive(listingUI[i].Listing.DoesListingMatchSearchTerm(searchTerm));
			}
			else
			{
				((Component)listingUI[i]).gameObject.SetActive(listingUI[i].Listing.DoesListingMatchCategoryFilter(categoryFilter) && listingUI[i].Listing.ShouldShow());
			}
		}
		for (int j = 0; j < listingUI.Count; j++)
		{
			((Component)listingUI[j]).transform.SetSiblingIndex(j);
		}
		List<ListingUI> list = listingUI.FindAll((ListingUI x) => !x.Listing.Item.IsUnlocked);
		list.Sort((ListingUI x, ListingUI y) => x.Listing.Item.RequiredRank.CompareTo(y.Listing.Item.RequiredRank));
		for (int num = 0; num < list.Count; num++)
		{
			((Component)list[num]).transform.SetAsLastSibling();
		}
		if (!listingUI.Exists((ListingUI x) => ((Component)x).gameObject.activeSelf))
		{
			DetailPanel.Close();
		}
		((MonoBehaviour)this).StartCoroutine(RefreshShownItemsNextFrame());
	}

	private IEnumerator RefreshShownItemsNextFrame()
	{
		yield return null;
		shopScreen.SetCurrentSelectedPanel(listingPanel, listingPanel.GetAValidCurrentSelectedSelectable(returnFirstFound: true));
	}
	...
	public virtual bool HandoverItems()
	{
		...
		for (int i = 0; i < list.Count; i++)
		{
			NetworkSingleton<VariableDatabase>.Instance.NotifyItemAcquired(((BaseItemDefinition)list[i].Item).ID, Cart.cartDictionary[list[i]]);
			int num = Cart.cartDictionary[list[i]];
			ItemInstance defaultInstance = list[i].Item.GetDefaultInstance();
			for (int j = 0; j < availableSlots.Count; j++)
			{
			...
			if (num > 0)
			{
				Debug.LogWarning((object)("Failed to handover all items in cart: " + ((BaseItemInstance)defaultInstance).Name));
				result = false;
			}
		}
		return result;
	...
	public List<ItemSlot> GetAvailableSlots()
	{
		...
		if ((Object)(object)loadingBayVehicle != (Object)null && Cart.LoadVehicleToggle.isOn)
		{
	...
	{
		if ((Object)(object)LoadingBayDetector != (Object)null && (Object)(object)LoadingBayDetector.closestVehicle != (Object)null && LoadingBayDetector.closestVehicle.IsPlayerOwned)
		{
	...
	public void PlaceItemInDeliveryBay(ItemInstance item)
	{
		...
		foreach (StorageEntity storageEntity in deliveryBays)
		{
			...
			if (num2 > 0)
			{
				...
				num -= ((BaseItemInstance)copy).Quantity;
			}
			if (num <= 0)
			{
				break;
	...
	{
		if (!((Object)(object)selectedListing == (Object)null) && selectedListing.Listing.Item.IsUnlocked)
		{
	...
	public void OpenAmountSelector(ListingUI listing)
	{
		...
		if (listing.Listing.Item.IsUnlocked && listing.CanAddToCart())
		{
			selectedListing = listing;
			((Component)AmountSelector).transform.position = ((Transform)listing.TopDropdownAnchor).position;
			amountSelectorMouseUp = false;
			AmountSelector.Open();
		}
	}
	...
	{
		selectedListing = listing;
		Singleton<UIScreenManager>.Instance.OpenPopupScreen("ModifyAmountMenu", "Add to Cart", "Add Amount", "", (float)defaultAddToCartAmount, (Action<float>)delegate(float amount)
		{
			QuantitySelectedNew((int)amount);
		}, null, UIPopupScreen_ModifyAmountMenu.ModifyAmountMenuMode.Store, (float)minAddToCartAmount, (float)addToCartTier1Amount, (float)addToCartTier2Amount, (float)addToCartTier3Amount, 3, listing.Listing);
	}

	private void QuantitySelectedNew(int amount)
	{
		if (!((Object)(object)selectedListing == (Object)null) && selectedListing.Listing.Item.IsUnlocked && amount != 0)
		{
			int quantity = Mathf.Clamp(amount, 1, selectedListing.Listing.IsUnlimitedStock ? 100000000 : selectedListing.Listing.CurrentStockMinusCart);
			Cart.AddItem(selectedListing.Listing, quantity);
			AddItemSound.Play();
	...
	public void Load(ShopData data)
	{
		...
		{
			ShopListing shopListing = Listings.Find((ShopListing x) => ((BaseItemDefinition)x.Item).ID == stockQuantity.String);
			if (shopListing == null)
			{
	...
	{
		return Listings.Find((ShopListing x) => ((BaseItemDefinition)x.Item).ID == itemID);
	}
	...
	public virtual ShopData GetSaveData()
	{
		...
		foreach (ShopListing listing in Listings)
		{
			if (!listing.IsUnlimitedStock && listing.CurrentStock != listing.DefaultStock)
			{
				list.Add(new StringIntPair(((BaseItemDefinition)listing.Item).ID, listing.CurrentStock));
			}
		}
