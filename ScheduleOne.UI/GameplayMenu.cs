...
public class GameplayMenu : Singleton<GameplayMenu>
{
	...
	protected override void Start()
	{
		...
		((Component)this).transform.localPosition = new Vector3(((Component)this).transform.localPosition.x, -2f, ((Component)this).transform.localPosition.z);
	...
	protected virtual void Update()
	{
		...
		{
			if (GameInput.GetCurrentInputDeviceIsGamepad() && PlayerSingleton<ScheduleOne.UI.Phone.Phone>.Instance.IsOpen)
			{
				return;
			}
		...
		void PrepAppOpen()
		{
			...
			if ((Object)(object)ScheduleOne.UI.Phone.Phone.ActiveApp != (Object)null)
			{
	...
	public void SetScreen(EGameplayScreen screen)
	{
		...
		if (CurrentScreen != screen)
		{
			...
			{
				((MonoBehaviour)Singleton<CoroutineService>.Instance).StopCoroutine(screenChangeRoutine);
			}
			Singleton<GameplayMenuInterface>.Instance.SetSelected(screen);
			screenChangeRoutine = ((MonoBehaviour)Singleton<CoroutineService>.Instance).StartCoroutine(ScreenChange());
		}
		IEnumerator ScreenChange()
		{
			...
			float startXPos = ((Component)this).transform.localPosition.x;
			for (float t = 0f; t < 0.12f; t += Time.deltaTime)
			{
				((Component)this).transform.localPosition = new Vector3(Mathf.Lerp(startXPos, endXPos, t / 0.12f), ((Component)this).transform.localPosition.y, ((Component)this).transform.localPosition.z);
				yield return (object)new WaitForEndOfFrame();
			}
			((Component)this).transform.localPosition = new Vector3(endXPos, ((Component)this).transform.localPosition.y, ((Component)this).transform.localPosition.z);
	...
	public void SetIsOpen(bool open)
	{
		...
		else
		{
			...
			if ((Object)(object)Player.Local.CurrentVehicle == (Object)null)
			{
		...
		{
			((MonoBehaviour)this).StopCoroutine(openCloseRoutine);
		}
		...
		openCloseRoutine = ((MonoBehaviour)this).StartCoroutine(SetIsOpenRoutine(open));
		IEnumerator SetIsOpenRoutine(bool flag)
		{
			...
			float num = 1f - ((Component)this).transform.localPosition.y / -2f;
			float adjustedLerpTime = 0.06f;
			float startVert = ((Component)this).transform.localPosition.y;
			float endVert;
			if (flag)
			{
			...
			{
				((Component)this).transform.localPosition = new Vector3(((Component)this).transform.localPosition.x, Mathf.Lerp(startVert, endVert, i / adjustedLerpTime), ((Component)this).transform.localPosition.z);
				yield return (object)new WaitForEndOfFrame();
			}
			((Component)this).transform.localPosition = new Vector3(((Component)this).transform.localPosition.x, endVert, ((Component)this).transform.localPosition.z);
			if (!flag)
			{
				((Behaviour)OverlayCamera).enabled = false;
				((Behaviour)OverlayLight).enabled = false;
