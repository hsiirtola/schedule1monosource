...
public class PassOutScreen : Singleton<PassOutScreen>
{
	...
	private void Continue()
	{
		...
		IEnumerator Routine()
		{
			...
			((Component)MainLabel).gameObject.SetActive(false);
			...
			((Component)Player.Local).transform.forward = child.forward;
			...
			{
				((TMP_Text)ContextLabel).text = "You awaken in a new location, unsure of how you got there.";
				if (cashLoss > 0f)
				{
					NetworkSingleton<MoneyManager>.Instance.ChangeCashBalance(0f - cashLoss);
					((TMP_Text)ContextLabel).text = ((TMP_Text)ContextLabel).text + "\n\n<color=#54E717>" + MoneyManager.FormatAmount(cashLoss) + "</color> is missing from your wallet.";
				}
				((Component)ContextLabel).gameObject.SetActive(true);
				for (float i = 0f; i < fadeTime; i += Time.deltaTime)
				{
	...
	public void Open()
	{
		...
		IEnumerator Routine()
		{
			((Component)MainLabel).gameObject.SetActive(true);
			((Component)ContextLabel).gameObject.SetActive(false);
			yield return (object)new WaitForSeconds(0.5f);
