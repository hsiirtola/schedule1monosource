...
public class RankUpCanvas : MonoBehaviour, IPostSleepEvent
{
	...
	public void StartEvent()
	{
		...
		UIScreen.SetCurrentSelectedPanel(LevelUpPanel);
		...
		Debug.Log((object)("XP Gain: " + xpGained));
		int num3 = xpGained;
		...
		((TMP_Text)ProgressLabel).text = num2 + " / " + NetworkSingleton<LevelManager>.Instance.GetXPForTier(fullRank.Rank) + " XP";
		((TMP_Text)OldRankLabel).text = FullRank.GetString(fullRank);
		UnlockedItemsCanvasGroup.alpha = 0f;
		coroutine = ((MonoBehaviour)Singleton<CoroutineService>.Instance).StartCoroutine(Routine());
		queuedRankUps.Clear();
		IEnumerator Routine()
		{
			...
			foreach (Tuple<FullRank, int, int> progress in progressDisplays)
			{
				...
				float lerpTime = Mathf.Lerp(0.5f, 1.5f, (float)(endXP - startXP) / (float)NetworkSingleton<LevelManager>.Instance.GetXPForTier(oldRank.Rank));
				...
				{
					int num5 = Mathf.RoundToInt(Mathf.Lerp((float)startXP, (float)endXP, i / lerpTime));
					ProgressSlider.value = (float)num5 / (float)xpForRank;
					((TMP_Text)ProgressLabel).text = num5 + " / " + xpForRank + " XP";
				...
				((TMP_Text)ProgressLabel).text = endXP + " / " + xpForRank + " XP";
				if (endXP == xpForRank)
				{
					ClickSound.Play();
	...
	public void EndEvent()
	{
		if (IsRunning)
		{
			...
			if (coroutine != null)
			{
				((MonoBehaviour)Singleton<CoroutineService>.Instance).StopCoroutine(coroutine);
				coroutine = null;
			}
	...
	private void PlayRankupAnimation(FullRank oldRank, FullRank newRank, bool playSound)
	{
		...
		((TMP_Text)OldRankLabel).text = FullRank.GetString(oldRank);
		((TMP_Text)NewRankLabel).text = FullRank.GetString(newRank);
		...
		for (int i = 0; i < UnlockedItems.Length; i++)
		{
			if (i < list.Count)
			{
				((Component)((Transform)UnlockedItems[i]).Find("Icon")).GetComponent<Image>().sprite = list[i].Icon;
				((TMP_Text)((Component)UnlockedItems[i]).GetComponentInChildren<TextMeshProUGUI>()).text = list[i].Title;
				((Component)UnlockedItems[i]).gameObject.SetActive(true);
			}
			else
			{
				((Component)UnlockedItems[i]).gameObject.SetActive(false);
			}
		}
		((TMP_Text)ExtraUnlocksLabel).text = ((list.Count > UnlockedItems.Length) ? ("+" + (list.Count - UnlockedItems.Length) + " more") : "");
		RankUpAnim.Play();
		if (playSound)
		{
			SoundEffect.Play();
