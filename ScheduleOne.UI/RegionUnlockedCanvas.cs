using System.Collections.Generic;
using System.Linq;
using ScheduleOne.DevUtilities;
using ScheduleOne.Economy;
using ScheduleOne.Map;
using ScheduleOne.NPCs;
using TMPro;
using UnityEngine;
using UnityEngine.UI;

namespace ScheduleOne.UI;

public class RegionUnlockedCanvas : Singleton<RegionUnlockedCanvas>, IPostSleepEvent
{
	public Animation OpenCloseAnim;

	public TextMeshProUGUI RegionLabel;

	public TextMeshProUGUI RegionDescription;

	public Image RegionImage;

	public UIScreen UIScreen;

	private EMapRegion region;

	public bool IsRunning { get; private set; }

	public int Order { get; private set; } = 5;

	public void QueueUnlocked(EMapRegion _region)
	{
		region = _region;
		Singleton<SleepCanvas>.Instance.AddPostSleepEvent(this);
	}

	public void StartEvent()
	{
		IsRunning = true;
		MapRegionData regionData = Singleton<ScheduleOne.Map.Map>.Instance.GetRegionData(region);
		((TMP_Text)RegionLabel).text = regionData.Name;
		RegionImage.sprite = regionData.RegionSprite;
		List<NPC> nPCsInRegion = NPCManager.GetNPCsInRegion(region);
		int num = nPCsInRegion.Count((NPC x) => (Object)(object)((Component)x).GetComponent<Customer>() != (Object)null);
		int num2 = nPCsInRegion.Count((NPC x) => x is Dealer && (x as Dealer).DealerType == EDealerType.PlayerDealer);
		int num3 = nPCsInRegion.Count((NPC x) => x is Supplier);
		((TMP_Text)RegionDescription).text = string.Empty;
		if (num > 0)
		{
			TextMeshProUGUI regionDescription = RegionDescription;
			((TMP_Text)regionDescription).text = ((TMP_Text)regionDescription).text + num + " potential customer" + ((num > 1) ? "s" : "");
		}
		if (num2 > 0)
		{
			if (((TMP_Text)RegionDescription).text.Length > 0)
			{
				TextMeshProUGUI regionDescription2 = RegionDescription;
				((TMP_Text)regionDescription2).text = ((TMP_Text)regionDescription2).text + "\n";
			}
			TextMeshProUGUI regionDescription3 = RegionDescription;
			((TMP_Text)regionDescription3).text = ((TMP_Text)regionDescription3).text + num2 + " dealer" + ((num2 > 1) ? "s" : "");
		}
		if (num3 > 0)
		{
			if (((TMP_Text)RegionDescription).text.Length > 0)
			{
				TextMeshProUGUI regionDescription4 = RegionDescription;
				((TMP_Text)regionDescription4).text = ((TMP_Text)regionDescription4).text + "\n";
			}
			TextMeshProUGUI regionDescription5 = RegionDescription;
			((TMP_Text)regionDescription5).text = ((TMP_Text)regionDescription5).text + num3 + " supplier" + ((num3 > 1) ? "s" : "");
		}
		OpenCloseAnim.Play("Rank up open");
		Singleton<UIScreenManager>.Instance.AddScreen(UIScreen);
	}

	public void EndEvent()
	{
		if (IsRunning)
		{
			OpenCloseAnim.Play("Rank up close");
			Singleton<UIScreenManager>.Instance.RemoveScreen(UIScreen);
			IsRunning = false;
		}
	}
}
