...
public class VehicleInitializer : NetworkBehaviour
{
	...
	public override void OnStartServer()
	{
		...
		if ((Object)(object)InitialParkingLot != (Object)null && !((Component)this).GetComponent<LandVehicle>().isParked)
		{
			int randomFreeSpotIndex = InitialParkingLot.GetRandomFreeSpotIndex();
			EParkingAlignment alignment = EParkingAlignment.RearToKerb;
			if (randomFreeSpotIndex != -1)
			{
				alignment = InitialParkingLot.ParkingSpots[randomFreeSpotIndex].Alignment;
			}
			((Component)this).GetComponent<LandVehicle>().Park(null, new ParkData(InitialParkingLot.GUID, randomFreeSpotIndex, alignment), network: false);
		}
	}
