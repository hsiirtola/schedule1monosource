...
public static class BatchingHelper
{
	...
	public static bool CanBeBatched(VolumetricLightBeamSD beamA, VolumetricLightBeamSD beamB, ref string reasons)
	{
		...
		if (Config.Instance.featureEnabledDynamicOcclusion && (Object)(object)((Component)beamA).GetComponent<DynamicOcclusionAbstractBase>() == (Object)null != ((Object)(object)((Component)beamB).GetComponent<DynamicOcclusionAbstractBase>() == (Object)null))
		{
			AppendErrorMessage(ref reasons, $"{((Object)beamA).name}/{((Object)beamB).name}: dynamically occluded and non occluded beams cannot be batched together");
			flag = false;
		}
	...
	public static bool CanBeBatched(VolumetricLightBeamSD beam, ref string reasons)
	{
		...
		{
			AppendErrorMessage(ref reasons, $"{((Object)beam).name} is not using shared mesh");
			result = false;
		}
		if (Config.Instance.featureEnabledDynamicOcclusion && (Object)(object)((Component)beam).GetComponent<DynamicOcclusionDepthBuffer>() != (Object)null)
		{
			AppendErrorMessage(ref reasons, $"{((Object)beam).name} is using the DynamicOcclusion DepthBuffer feature");
			result = false;
		}
		return result;
	}
	...
	{
		bool result = true;
		if (Config.Instance.featureEnabledShadow && (Object)(object)beam.GetAdditionalComponentShadow() != (Object)null)
		{
			AppendErrorMessage(ref reasons, $"{((Object)beam).name} is using the Shadow feature");
			result = false;
		}
		if (Config.Instance.featureEnabledCookie && (Object)(object)beam.GetAdditionalComponentCookie() != (Object)null)
		{
			AppendErrorMessage(ref reasons, $"{((Object)beam).name} is using the Cookie feature");
			result = false;
		}
		return result;
	}
