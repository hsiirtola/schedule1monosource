...
public class LightGenerator : MonoBehaviour
{
	...
	public void Generate()
	{
		...
		for (int i = 0; i < CountX; i++)
		{
			...
			{
				GameObject val = null;
				val = ((!AddLight) ? new GameObject("Light_" + i + "_" + j, new Type[2]
				{
					typeof(VolumetricLightBeamSD),
					typeof(Rotater)
				}) : new GameObject("Light_" + i + "_" + j, new Type[3]
				{
					typeof(Light),
					typeof(VolumetricLightBeamSD),
					typeof(Rotater)
				}));
				val.transform.SetPositionAndRotation(new Vector3((float)i * OffsetUnits, PositionY, (float)j * OffsetUnits), Quaternion.Euler((float)Random.Range(-45, 45) + 90f, (float)Random.Range(0, 360), 0f));
				VolumetricLightBeamSD component = val.GetComponent<VolumetricLightBeamSD>();
				if (AddLight)
				{
					Light component2 = val.GetComponent<Light>();
					component2.type = (LightType)0;
				...
				val.GetComponent<Rotater>().EulerSpeed = new Vector3(0f, (float)Random.Range(-500, 500), 0f);
			}
		}
	}
}
